...
| `email.received`          | Inbound email was received on your domain            |
...
Includes details for emails received via your configured inbound domain:

```typescript
{
  id: string;               // Inbound email ID
  provider: "ses";          // Inbound provider
  ...
  domainId?: number | null; // Matched domain ID
  attachments: Array<{
    id: string;
